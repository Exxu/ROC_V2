...
std::size_t Serial::readByteTimed(uint8_t* byte, unsigned int timeout_ms)
{
  if (byte == nullptr)
    return 0;

  return readBytesTimed(byte, 1, timeout_ms);
}

std::size_t Serial::readBytesTimed(uint8_t* buffer, std::size_t nbytes, unsigned int timeout_ms)
{
  if (buffer == nullptr || nbytes == 0 || !isOpen() || _io == nullptr || _port == nullptr)
    return 0;
  ...
  try
  {
    ...
    timer.async_wait([this](const boost::system::error_code& ec) {
      if (!ec && _port != nullptr && _port->is_open())
      {
        boost::system::error_code ignored;
        _port->cancel(ignored);
      }
    });

    boost::asio::async_read(
      *_port,
      boost::asio::buffer(buffer, nbytes),
      boost::asio::transfer_exactly(nbytes),
  ...
  {
    logException("readBytesTimed", e.what());
    return 0;
  }
