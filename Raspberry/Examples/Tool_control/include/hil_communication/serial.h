...
public:
  ...
  // Timed reads. These return 0 on timeout, error, or closed port.
  std::size_t readByteTimed(uint8_t* byte, unsigned int timeout_ms);
  std::size_t readBytesTimed(uint8_t* buffer, std::size_t nbytes, unsigned int timeout_ms);
