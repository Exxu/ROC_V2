...
--
-- Freno automatico usando RC1 + RC2 + RC5 y RELAY3/RELAY4.
-- Incluye bloqueo de seguridad por estado ARM/DISARM.
--
-- Entradas:
--   RC1 = giro izquierda/derecha
--   RC2 = adelante/atras
--   RC5 = habilita/desfrena ambos frenos cuando esta ON
--
-- Salidas:
...
--         -> se permite liberar ambos frenos, pero solo si RC5 esta ON
--
--   RC5 OFF:
--      -> ambos frenos ON siempre
--
--   RC5 ON:
--      -> permite liberar ambos frenos si el Rover esta armado
--         y RC1 o RC2 estan fuera de banda muerta
--
-- IMPORTANTE:
--   Deja RC5_OPTION = 0
--   El script lee RC5, pero ArduPilot no debe accionarlo directamente.
--
--   RC6 ya no se usa para controlar los frenos.
--   Se recomienda dejar RC6_OPTION = 0 si no se va a usar para otra funcion.
--
...
-- Hardware verificado con logica invertida:
--   enable_brake = true  -> relay:off()
--   enable_brake = false -> relay:on()
local BRAKE_ACTIVE_HIGH = false

-- Switch global de seguridad:
local RC_BRAKE_ENABLE = 5      -- RC5 habilita/libera ambos frenos
local ENABLE_PWM = 1700        -- Switch ON si RC5 >= 1700 us
...
function update()
    if not warned then
        ...
        gcs:send_text(6, "Lua brake: armado + RC1/RC2 + RC5 global")
        warned = true
    end
    ...
    local left_enabled = switch_is_on(RC_BRAKE_ENABLE)
    local right_enabled = switch_is_on(RC_BRAKE_ENABLE)
